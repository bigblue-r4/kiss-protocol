...
1. **[RESOLVED] Pipelock maintenance:** same operator. Phase 6 coordination is internal — tag a release, expose a stable Go API, vendor. No external contract needed.
...
4. **[RESOLVED] Mirror policy:** Pure BYO for v2.0. Installer does not set `mirror_url`. Phase 5 ships the mechanism only; operator provides the endpoint. Document in `docs/mirror-setup.md`.
